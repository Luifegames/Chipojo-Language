...
int is_truthy(Value v)
{
    if (v.type == VAR_NUMBER)
        return v.value.num != 0;
    if (v.type == VAR_STRING)
        return v.value.str[0] != '\0';
...
Value parse_postfix(Value base)
{
    while (1)
    {
        if (current_token.type == TOKEN_LEFTBRACKET) // Get list value
        {
            ...
            base = get_list(base.value.list, (int)idx.value.num);
...
Value get_property(Value object, char *property, int line)
{
    ...
    {
        return get_dict(object.value.dict, property);
    }
    ...
    if (object.type == VAR_STRING)
    {
        if (strcmp(property, "length") == 0)
        {
            ...
            v.value.num = strlen(object.value.str);
            return v;
        }
...
Value function_call(Value func_val, Value *args_v, int count, int last_index)
{
    ...
    {
        set_variable(func_val.value.func.param[x], args_v[x]);
    }
...
void assignation()
{
    ...
    {
        strcpy(name, current_token.name);
        if (peek_next_token_type() == TOKEN_PARENTLEFT)
        { // is a function
            factor();
            return;
        }
...
void print_dict(Dict *dict)
{
    ...
    for (int e = 0; e < dict->count; e++)
    {
        ...
        case VAR_NULL:
            ...
            break;
        case VAR_DICT:
            print_dict(dict->entries[e].value->value.dict);
            break;
        case VAR_LIST:
            print_list(dict->entries[e].value->value.list);
            break;
...
void print_list(List *list)
{
    ...
    for (int e = 0; e < list->count; e++)
    {
        ...
        case VAR_NULL:
            ...
            break;
        case VAR_DICT:
            print_dict(list->items[e].value.dict);
            break;
        case VAR_LIST:
            print_list(list->items[e].value.list);
            break;
